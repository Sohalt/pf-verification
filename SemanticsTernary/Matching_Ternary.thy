...
imports Iptables_Semantics.Ternary "../Firewall_Common"
begin
...
fun ternary_to_bool_unknown_match_tac :: "'packet unknown_match_tac \<Rightarrow> action \<Rightarrow> decision \<Rightarrow> 'packet \<Rightarrow> ternaryvalue \<Rightarrow> bool" where
  ...
  "ternary_to_bool_unknown_match_tac _ _ _ _ TernaryFalse = False" |
  "ternary_to_bool_unknown_match_tac \<alpha> a d p TernaryUnknown = \<alpha> a d p"
...
\end{enumerate}
\<close>
definition matches :: "('a, 'packet) match_tac \<Rightarrow> 'a match_expr \<Rightarrow> action \<Rightarrow> decision \<Rightarrow> 'packet \<Rightarrow> bool" where
  "matches \<gamma> m a d p \<equiv> ternary_to_bool_unknown_match_tac (snd \<gamma>) a d p (ternary_ternary_eval (map_match_tac (fst \<gamma>) p m))"

text\<open>Alternative matches definitions, some more or less convenient\<close>

lemma matches_tuple: "matches (\<beta>, \<alpha>) m a d p = ternary_to_bool_unknown_match_tac \<alpha> a d p (ternary_ternary_eval (map_match_tac \<beta> p m))"
unfolding matches_def by simp

lemma matches_case: "matches \<gamma> m a d p \<longleftrightarrow> (case ternary_eval (map_match_tac (fst \<gamma>) p m) of None \<Rightarrow> (snd \<gamma>) a d p | Some b \<Rightarrow> b)"
unfolding matches_def ternary_eval_def
by (cases "(ternary_ternary_eval (map_match_tac (fst \<gamma>) p m))") auto

lemma matches_case_tuple: "matches (\<beta>, \<alpha>) m a d p \<longleftrightarrow> (case ternary_eval (map_match_tac \<beta> p m) of None \<Rightarrow> \<alpha> a d p | Some b \<Rightarrow> b)"
by (auto simp: matches_case split: option.splits)

lemma matches_case_ternaryvalue_tuple: "matches (\<beta>, \<alpha>) m a d p \<longleftrightarrow> (case ternary_ternary_eval (map_match_tac \<beta> p m) of 
        TernaryUnknown \<Rightarrow> \<alpha> a d p | 
...
lemma matches_casesE:
  "matches (\<beta>, \<alpha>) m a d p \<Longrightarrow> 
    (ternary_ternary_eval (map_match_tac \<beta> p m) = TernaryUnknown \<Longrightarrow> \<alpha> a d p \<Longrightarrow> P) \<Longrightarrow> 
...
\<close>
lemma "\<lbrakk> ternary_ternary_eval (map_match_tac \<beta> p expr) = TernaryUnknown \<rbrakk> \<Longrightarrow> matches (\<beta>, \<alpha>) expr a d p \<longleftrightarrow> matches (\<beta>, \<alpha>) (MatchNot expr) a d p"
by(simp add: matches_case_ternaryvalue_tuple)


lemma bunch_of_lemmata_about_matches:
  "matches \<gamma> (MatchAnd m1 m2) a d p \<longleftrightarrow> matches \<gamma> m1 a d p \<and> matches \<gamma> m2 a d p" (*split AND*)
  "matches \<gamma> MatchAny a d p" (*MatchAny is True*)
  "matches \<gamma> (MatchNot MatchAny) a d p \<longleftrightarrow> False" (*Not True*)
  "matches \<gamma> (MatchNot (MatchNot m)) a d p \<longleftrightarrow> matches \<gamma> m a d p" (*idempotence*)
proof(case_tac [!] \<gamma>)
qed (simp_all split: ternaryvalue.split add: matches_case_ternaryvalue_tuple)

lemma match_raw_bool:
  "matches (\<beta>, \<alpha>) (Match expr) a d p = (case ternary_to_bool (\<beta> expr p) of Some r \<Rightarrow> r | None \<Rightarrow> (\<alpha> a d p))" (*Match raw*)
by(simp_all split: ternaryvalue.split add: matches_case_ternaryvalue_tuple)
lemma match_raw_ternary:
  "matches (\<beta>, \<alpha>) (Match expr) a d p = (case (\<beta> expr p) of TernaryTrue \<Rightarrow> True | TernaryFalse \<Rightarrow> False | TernaryUnknown \<Rightarrow> (\<alpha> a d p))" (*Match raw explicit*)
by(simp_all split: ternaryvalue.split add: matches_case_ternaryvalue_tuple)

(*kind of the DeMorgan Rule for matches*)
lemma matches_DeMorgan: "matches \<gamma> (MatchNot (MatchAnd m1 m2)) a d p \<longleftrightarrow> (matches \<gamma> (MatchNot m1) a d p) \<or> (matches \<gamma> (MatchNot m2) a d p)"
...
lemma matches_and_comm: "matches \<gamma> (MatchAnd m m') a d p \<longleftrightarrow> matches \<gamma> (MatchAnd m' m) a d p"
apply(cases \<gamma>, rename_tac \<beta> \<alpha>, clarify)
by(simp add: matches_case_ternaryvalue_tuple eval_ternary_And_comm)

lemma matches_not_idem: "matches \<gamma> (MatchNot (MatchNot m)) a d p \<longleftrightarrow> matches \<gamma> m a d p"
by (fact bunch_of_lemmata_about_matches)


lemma MatchOr: "matches \<gamma> (MatchOr m1 m2) a d p \<longleftrightarrow> matches \<gamma> m1 a d p \<or> matches \<gamma> m2 a d p"
  by(simp add: MatchOr_def matches_DeMorgan matches_not_idem)

lemma MatchOr_MatchNot: "matches \<gamma> (MatchNot (MatchOr m1 m2)) a d p \<longleftrightarrow> matches \<gamma> (MatchNot m1) a d p \<and> matches \<gamma> (MatchNot m2) a d p"
...
context
begin
  private lemma matches_simp1: "matches \<gamma> m a d p \<Longrightarrow> matches \<gamma> (MatchAnd m m') a d p \<longleftrightarrow> matches \<gamma> m' a d p"
    apply(cases \<gamma>, rename_tac \<beta> \<alpha>, clarify)
    apply(simp split: ternaryvalue.split_asm ternaryvalue.split add: matches_case_ternaryvalue_tuple)
    done
  
  private lemma matches_simp11: "matches \<gamma> m a d p \<Longrightarrow> matches \<gamma> (MatchAnd m' m) a d p \<longleftrightarrow> matches \<gamma> m' a d p"
    by(simp_all add: matches_and_comm matches_simp1)
  
  private lemma matches_simp2: "matches \<gamma> (MatchAnd m m') a d p \<Longrightarrow> \<not> matches \<gamma> m a d p \<Longrightarrow> False"
    by (simp add: bunch_of_lemmata_about_matches)
  private lemma matches_simp22: "matches \<gamma> (MatchAnd m m') a d p \<Longrightarrow> \<not> matches \<gamma> m' a d p \<Longrightarrow> False"
    by (simp add: bunch_of_lemmata_about_matches)
  
  (*m simplifies to MatchUnknown*)
 private  lemma matches_simp3: "matches \<gamma> (MatchNot m) a d p \<Longrightarrow> matches \<gamma> m a d p \<Longrightarrow> (snd \<gamma>) a d p"
    apply(cases \<gamma>, rename_tac \<beta> \<alpha>, clarify)
    apply(simp split: ternaryvalue.split_asm ternaryvalue.split add: matches_case_ternaryvalue_tuple)
    done
  private lemma "matches \<gamma> (MatchNot m) a d p \<Longrightarrow> matches \<gamma> m a d p \<Longrightarrow> (ternary_eval (map_match_tac (fst \<gamma>) p m)) = None"
    apply(cases \<gamma>, rename_tac \<beta> \<alpha>, clarify)
...
lemma matches_iff_apply_f_generic: "ternary_ternary_eval (map_match_tac \<beta> p (f (\<beta>,\<alpha>) a m)) = ternary_ternary_eval (map_match_tac \<beta> p m) \<Longrightarrow> matches (\<beta>,\<alpha>) (f (\<beta>,\<alpha>) a m) a d p \<longleftrightarrow> matches (\<beta>,\<alpha>) m a d p"
  by(simp split: ternaryvalue.split_asm ternaryvalue.split add: matches_case_ternaryvalue_tuple)

lemma matches_iff_apply_f: "ternary_ternary_eval (map_match_tac \<beta> p (f m)) = ternary_ternary_eval (map_match_tac \<beta> p m) \<Longrightarrow> matches (\<beta>,\<alpha>) (f m) a d p \<longleftrightarrow> matches (\<beta>,\<alpha>) m a d p"
  by(fact matches_iff_apply_f_generic)

subsection\<open>Removing Unknown Primitives\<close>

definition unknown_match_all :: "'a unknown_match_tac \<Rightarrow> action \<Rightarrow> decision \<Rightarrow> bool" where
   "unknown_match_all \<alpha> a d = (\<forall>p. \<alpha> a d p)"
definition unknown_not_match_any :: "'a unknown_match_tac \<Rightarrow> action \<Rightarrow> decision \<Rightarrow> bool" where
   "unknown_not_match_any \<alpha> a d = (\<forall>p. \<not> \<alpha> a d p)"

(*see upper_closure_matchexpr*)
fun remove_unknowns_generic :: "('a, 'packet) match_tac \<Rightarrow> action \<Rightarrow> decision \<Rightarrow> 'a match_expr \<Rightarrow> 'a match_expr" where
  "remove_unknowns_generic _ _ _ MatchAny = MatchAny" |
  "remove_unknowns_generic _ _ _ (MatchNot MatchAny) = MatchNot MatchAny" |
  "remove_unknowns_generic (\<beta>, \<alpha>) a d (Match A) = (if
      (\<forall>p. ternary_ternary_eval (map_match_tac \<beta> p (Match A)) = TernaryUnknown)
    then
      if unknown_match_all \<alpha> a d then MatchAny else if unknown_not_match_any \<alpha> a d then MatchNot MatchAny else Match A
    else (Match A))" |
  "remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot (Match A)) = (if
      (\<forall>p. ternary_ternary_eval (map_match_tac \<beta> p (Match A)) = TernaryUnknown)
    then
      if unknown_match_all \<alpha> a d then MatchAny else if unknown_not_match_any \<alpha> a d then MatchNot MatchAny else MatchNot (Match A)
    else MatchNot (Match A))" |
  "remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot (MatchNot m)) = remove_unknowns_generic (\<beta>, \<alpha>) a d m" |
  "remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchAnd m1 m2) = MatchAnd
  ...
  "remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot (MatchAnd m1 m2)) = 
    (if (remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot m1)) = MatchAny \<or>
        ...
        then MatchAny else 
        (if (remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot m1)) = MatchNot MatchAny then 
          remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot m2) else
         if (remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot m2)) = MatchNot MatchAny then 
          remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot m1) else
         MatchNot (MatchAnd (MatchNot (remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot m1))) (MatchNot (remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot m2)))))
       )"

lemma[code_unfold]: "remove_unknowns_generic \<gamma> a d (MatchNot (MatchAnd m1 m2)) = 
    (let m1' = remove_unknowns_generic \<gamma>  a d (MatchNot m1); m2' = remove_unknowns_generic \<gamma>  a d (MatchNot m2) in
...
lemma remove_unknowns_generic_simp_3_4_unfolded: "remove_unknowns_generic (\<beta>, \<alpha>) a d (Match A) = (if
      (\<forall>p. ternary_ternary_eval (map_match_tac \<beta> p (Match A)) = TernaryUnknown)
    then
      if (\<forall>p. \<alpha> a d p) then MatchAny else if (\<forall>p. \<not> \<alpha> a d p) then MatchNot MatchAny else Match A
    else (Match A))" 
 "remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot (Match A)) = (if
      (\<forall>p. ternary_ternary_eval (map_match_tac \<beta> p (Match A)) = TernaryUnknown)
    then
      if (\<forall>p. \<alpha> a d p) then MatchAny else if (\<forall>p. \<not> \<alpha> a d p) then MatchNot MatchAny else MatchNot (Match A)
    else MatchNot (Match A))"
...
lemma "matches (\<beta>, \<alpha>) (remove_unknowns_generic (\<beta>, \<alpha>) a d (MatchNot (Match A))) a d p = matches (\<beta>, \<alpha>) (MatchNot (Match A)) a d p"
by(simp add: remove_unknowns_generic_simps2 matches_case_ternaryvalue_tuple)



lemma remove_unknowns_generic: "matches \<gamma> (remove_unknowns_generic \<gamma> a d m) a d = matches \<gamma> m a d"
proof -
  have "matches \<gamma> (remove_unknowns_generic \<gamma> a d m) a d p = matches \<gamma> m a d p"
  for p
  proof(induction \<gamma> a d m rule: remove_unknowns_generic.induct)
  case 3 thus ?case
...
lemma remove_unknowns_generic_specification: "a=Pass \<or> a=Block \<Longrightarrow> packet_independent_\<alpha> \<alpha> \<Longrightarrow>
  packet_independent_\<beta>_unknown \<beta> \<Longrightarrow>
   \<not> has_unknowns \<beta> (remove_unknowns_generic (\<beta>, \<alpha>) a d m)"
  proof(induction "(\<beta>, \<alpha>)" a d m rule: remove_unknowns_generic.induct)
...
begin
  ...
  private lemma no_primitives_matchNot: assumes "\<not> has_primitive m" shows "matches \<gamma> (MatchNot m) a d p \<longleftrightarrow> \<not> matches \<gamma> m a d p"
  proof -
    obtain \<beta> \<alpha> where "(\<beta>, \<alpha>) = \<gamma>" by (cases \<gamma>, simp)
    thm no_primitives_no_unknown
    from assms have "matches (\<beta>, \<alpha>) (MatchNot m) a d p \<longleftrightarrow> \<not> matches (\<beta>, \<alpha>) m a d p"
      apply(induction m)
  ...
  lemma matcheq_matchAny: "\<not> has_primitive m \<Longrightarrow> matcheq_matchAny m \<longleftrightarrow> matches \<gamma> m a d p"
  proof(induction m)
  ...
  case (MatchNot m)
    ...
    with no_primitives_matchNot have "matches \<gamma> (MatchNot m) a d p = (\<not> matches \<gamma> m a d p)" by metis
  ...
  lemma matcheq_matchNone_not_matches: "matcheq_matchNone m \<Longrightarrow> \<not> matches \<gamma> m a d p"
...
lemma matches_MatchNot_no_unknowns:
   ...
   shows "matches (\<beta>,\<alpha>) (MatchNot m) a d p \<longleftrightarrow> \<not> matches (\<beta>,\<alpha>) m a d p"
proof -
...
lemma MatchNot_ternary_ternary_eval: "(ternary_ternary_eval (map_match_tac \<beta> p m')) = (ternary_ternary_eval (map_match_tac \<beta> p m)) \<Longrightarrow>
    matches (\<beta>,\<alpha>) (MatchNot m') a d p = matches (\<beta>,\<alpha>) (MatchNot m) a d p"
...
lemma matches_induction_case_MatchNot:
      assumes "\<alpha> Block d \<noteq> \<alpha> Pass d" and "packet_independent_\<alpha> \<alpha>"
      and     "\<forall> a. matches (\<beta>,\<alpha>) m' a d p = matches (\<beta>,\<alpha>) m a d p"
      shows   "matches (\<beta>,\<alpha>) (MatchNot m') a d p = matches (\<beta>,\<alpha>) (MatchNot m) a d p"
proof -
  from assms(1) assms(2) have xxxx_xxX: "\<And>b. \<forall>a. \<alpha> a d p = (\<not> b) \<Longrightarrow> False"
    apply(simp add: packet_independent_\<alpha>_def)
    apply(case_tac "\<alpha> Pass d p")
     apply(simp_all)
     apply(case_tac [!] "\<alpha> Block d p")
